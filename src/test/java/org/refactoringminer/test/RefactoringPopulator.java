...
public class RefactoringPopulator {
	...
	private static void prepareFSERefactorings(TestBuilder test, BigInteger flag)
		...
		for (Root root : roots) {
			TestBuilder.ProjectMatcher.CommitMatcher commitMatcher = test.project(root.repository, "master").atCommit(root.sha1);
			commitMatcher.containsOnly(extractRefactorings(root.refactorings));
			commitMatcher.containsPureRefactorings(extractPureRefactorings(root.refactorings));
		}
	}

	public static String[] extractPureRefactorings(List<Refactoring> refactoring) {
		int count = 0;
		for (Refactoring ref : refactoring) {
			if (ref.validation.contains("TP") && StringUtils.equals(ref.purity.purityValue, "1"))
				count++;
		}
	...
	public static List<Root> getFSERefactorings(BigInteger flag) throws JsonParseException, JsonMappingException, IOException {
		...
		String jsonFile = EXPECTED_PATH + "Puritydata.json";
		...
		for (Root root : roots) {
			if(!deletedCommits.contains(root.sha1)) {
				...
				root.refactorings.forEach((refactoring) -> {
					if (RefactoringSet.isAdded(refactoring, flag))
	...
	public static void printRefDiffResults(BigInteger flag) {
		...
		try {
			...
			Tuple[] tmp = {};
			System.out.println("Total\t" + ResultComparator.buildResultMessage(result.values().toArray(tmp)));
			for (String key : result.keySet()) {
				System.out.println(getInitials(key) + "\t" + ResultComparator.buildResultMessage(result.get(key)));
			}
